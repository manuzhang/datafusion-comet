...
import org.apache.spark.sql.comet.{CometNativeScanExec, CometScanExec, CometSparkToColumnarExec}
...
abstract class ParquetReadSuite extends CometTestBase {
  ...
  test("spark.sql.parquet.binaryAsString falls back from native scan") {
    withTempDir { dir =>
      ...
      Seq(
        Binary.fromString("one"),
        Binary.fromConstantByteArray(Array(0xff.toByte)),
        Binary.fromConstantByteArray(Array(0xfe.toByte))).zipWithIndex
      ...
      withSQLConf(
        ...
        val (_, cometPlan) = checkSparkAnswerAndFallbackReason(df, fallbackReason)
        assert(
          collect(cometPlan) { case _: CometNativeScanExec => true }.isEmpty,
          s"Native scan should fall back with binaryAsString enabled:\n${cometPlan.treeString}")
        assert(
          collect(cometPlan) { case _: CometSparkToColumnarExec => true }.isEmpty,
          s"Spark-to-Arrow should fall back for binaryAsString data:\n${cometPlan.treeString}")

        // Spark StringType preserves malformed bytes for SQL evaluation. A lossy UTF-8 conversion
        // at any Arrow boundary would collapse these two distinct values to the same U+FFFD
        // string.
        val (_, aggregateCometPlan) =
          checkSparkAnswerAndFallbackReason(df.selectExpr("count(distinct value)"), fallbackReason)
        assert(
          collect(aggregateCometPlan) { case _: CometShuffleExchangeExec => true }.isEmpty,
          s"Comet shuffle should fall back for binaryAsString data:\n${aggregateCometPlan.treeString}")
      ...
      withSQLConf(
        ...
        CometConf.COMET_CONVERT_FROM_PARQUET_ENABLED.key -> "true") {
        val v2Aggregate =
          spark.read.parquet(path.toString).selectExpr("count(distinct value)")
        val (_, v2CometPlan) = checkSparkAnswer(v2Aggregate)
        assert(
          collect(v2CometPlan) { case _: CometSparkToColumnarExec => true }.isEmpty,
          s"Spark-to-Arrow should fall back for V2 binaryAsString data:\n${v2CometPlan.treeString}")
        assert(
          collect(v2CometPlan) { case _: CometShuffleExchangeExec => true }.isEmpty,
          s"Comet shuffle should fall back for V2 binaryAsString data:\n${v2CometPlan.treeString}")
      }
    }
  }
